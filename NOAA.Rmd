...
Large summer storm | HURRICANE
Tidal / Coastal | TSUNAMI or SURGE or TIDE or SURF or SEAS
Lightning | LIGHTNING
Wind | WIND
Hail | HAIL
Flooding | FLOOD or WATER or RAIN
Winter storm / Cold | BLIZZARD or SNOW or COLD or FREEZE or CHILL or ICE or HYPOTHERMIA or SLEET
Heat / Fire | HEAT or DROUGHT or FIRE
...
classifyEventType <- function(s) {
	...
	else if (grepl("HURRICANE|TROPICAL STORM", s))
	{
		result <- "Hurricane"
	}
	else if (grepl("TSUNAMI|SURGE|TIDE|SURF|SEAS|MARINE|RIP CURRENT", s))
	{
	...
	else if (grepl("BLIZZARD|SNOW|COLD|FREEZE|CHILL|ICE|HYPOTHERMIA|SLEET|WINTER|LOW TEMP|RECORD LOW", s))
	{
		result <- "Winter Storm / Cold"
	}
	else if (grepl("HEAT|DROUGHT|FIRE|HIGH TEMP|RECORD HIGH", s))
	{
		result <- "Heat / Fire"
	}
	else if (grepl("DUST|FOG", s))
	{
		result <- "Dust / Fog"
	}
